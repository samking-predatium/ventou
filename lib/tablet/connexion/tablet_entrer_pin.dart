...
class TabletEntrerPin extends StatefulWidget {
  const TabletEntrerPin({super.key});

  @override
  State<TabletEntrerPin> createState() => _TabletEntrerPinState();
}

class _TabletEntrerPinState extends State<TabletEntrerPin> {
    String? _enteredPin;
  bool _hasError = false;
  String _errorMessage = '';
  int _pinAttempts = 0;
  bool _isProcessing = false;
  final _encryptionService = EncryptionService(); // Initialize the encryption service
        ...
        if (mounted) {
          ...
          if (context.mounted) {
            ...
            await Navigator.push(
              context,
              SlidePageRoute(page: TabletFirstScreen()),
            );
          }
        }
      } else {
                      ...
                      if (!_isProcessing) ...[
                        ...
                        TextButton(
                          onPressed: () {
                            ...
                            navigator.push('/tablet-verifier-compte');
                          },
                          child: const Text(
                            'Code pin oublié ?',
                            style: TextStyle(
